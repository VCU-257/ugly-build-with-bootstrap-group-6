...
- **`Oversight`** We started this thinking ahead and just doing .js files for future use, but kept the core use of bootstrap function present throughout. I then realize if we needed github pages we needed to have a way to deploy it through github actions which I have never done CI/CD pipeline through github before so I had to look into that. 

## 4. AI / LLM Usage

*Did you use an AI tool to help write or debug your code?*

* **What I asked the AI:**  how to structure a **Bootstrap card** with a **responsive image** (**`ratio`** + **`object-fit-cover`**). Look over my github deployment yml file and learn from the feedback on how to set it up, along with vite.

* **How it helped & What I learned:** After suggesting the exact bootstrap components it was more about ensure I connected it all together properly. I walked through the generated JSX in the repo, ran **`npm run dev`**, along with a great tool such as **`vite`** to verify, and adjusted copy/structure to match our **team plan** and course requirements so I could explain every class and tag in a review.
* 
...
* I worked on the Home page at root '/' and the listing cards that appeard on them, I also assisted in the Navbar global component. Also due to my own doings had to github deployement.
